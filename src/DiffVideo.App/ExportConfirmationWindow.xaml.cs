...
public partial class ExportConfirmationWindow : Window
{
    ...
    public ExportConfirmationWindow(Composition snapshot)
    {
        ...
        AddSummarySection("레이어",
            $"위에 표시  {(snapshot.Video1.ZIndex > snapshot.Video2.ZIndex ? "A · 영상 1" : "B · 영상 2")}");
        AddSummarySection("영상 1", Describe(snapshot.Video1));
        AddSummarySection("영상 2", Describe(snapshot.Video2));
        AddSummarySection("MP3", snapshot.ExtraAudio is { } audio
            ? $"{Path.GetFileName(audio.Media.Path)}\n시작 {audio.Start.TotalSeconds:0.000}초 · 오디오 {Audio(audio.IncludeAudio, audio.Volume)}"
            : "없음");
