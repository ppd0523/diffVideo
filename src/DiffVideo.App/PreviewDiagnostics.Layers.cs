...
internal static partial class PreviewDiagnostics
{
    private static async Task CheckLayersAsync(MainWindow window, string[] files, string report)
    {
        ...
        Assert(vm.Video1.ZIndex == 1 && vm.Video2.ZIndex == 0, "Bring-forward button updates model");
        ...
        Assert(new System.Windows.Documents.TextRange(confirmation.SummaryText.ContentStart, confirmation.SummaryText.ContentEnd).Text.Contains("위에 표시  A · 영상 1"), "Confirmation names front video");
        confirmation.Close();
        window.UpdateLayout();
        ...
        Assert(vm.Video2.ZIndex == 1 && vm.Video1.ZIndex == 0 && ReferenceEquals(sourceB, SourceAt(4)), "Send-backward button reverses actual preview order");
        ...
        Assert(vm.FrontVideoIndex == 1, "Existing forward action stays synchronized");
        vm.MoveSelectedLayer(false);
        Assert(vm.FrontVideoIndex == 2, "Existing backward action stays synchronized");
        ...
        {
            "Layer buttons, model and hit-target order", "Actual preview drawing order and FFmpeg export order",
